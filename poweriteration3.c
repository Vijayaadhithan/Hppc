...
EigenResult powerIteration(const Matrix* A, const Vector* x, double epsilon) {
    ...
    do {
        #pragma omp parallel for shared(A, currentVector, z) private(i, j) num_threads(10)
        for (i = 0; i < A->size; i++) {
            z[i] = 0.0;
            for (j = 0; j < A->size; j++) {
        ...
        #pragma omp parallel for shared(z, invZmax) private(i) num_threads(10)
        for (i = 0; i < A->size; i++) {
            z[i] *= invZmax;
        }

        #pragma omp parallel for shared(z, currentVector, e) private(i) num_threads(10)
        for (i = 0; i < A->size; i++) {
        ...
        #pragma omp parallel for shared(currentVector, z) private(i) num_threads(10)
        for (i = 0; i < A->size; i++) {
